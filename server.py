...
def trainModel(model, train_data):
    ...
    model.fit({'input': X}, {'targets': Y}, n_epoch=45, validation_set=({'input': test_x}, {'targets': test_y}), snapshot_step=500, show_metric=True, run_id=MODEL_NAME)
...
def main():
    ...
    with tf.Session() as sess:
        ...
        tfvars_values = sess.run(tfvars)
        
        with open('output-file.txt', 'w') as f:
            for var, vals in zip(tfvars, tfvars_values):
                f.write("{} , {}".format(var.name, vals))
